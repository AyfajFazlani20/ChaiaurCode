...
*/
    ...
    handleDateClick(event) {
        ...
        const isClickable = JSON.parse(event.currentTarget.dataset.clickable);
    
        if (!isClickable) {
            return; // Do nothing for non-clickable dates
        }
    ...
    generateCalendarDays(year, month) {
        ...
        for (let i = 1; i <= daysInMonthCount; i++) {
            ...
            const totalMinutes = this.totalMinutesByDate[dateString] || 0;
            const dayOfWeek = date.getDay();
    
            let classes = [];
            let isClickable = true;
    
            if (companyHolidays.includes(dateString)) {
                classes.push('holiday-yellow');
                isClickable = false;
            } else if (dayOfWeek === 0 || dayOfWeek === 6) {
                classes.push('weekend-grey');
                isClickable = false;
            } else if (date < today) {
                classes.push(totalMinutes < 480 ? 'color-red' : 'color-green');
            } else {
                // Future date
                isClickable = this.isAllowedFutureEntry(date, this.taskName, this.projectName);
            }
    ...
    saveLog(event) {
        ...
        if (!this.isAllowedFutureEntry(this.selectedDate, this.taskName, this.projectName)) {
            this.showWarning = true;
            this.warningMessage = "You can only log 'Leave' for 'Elastik Teams' project on future dates";
            setTimeout(() => {
                this.showWarning = false;
            }, 2000);
            return;
   ...
   isAllowedFutureEntry(date, taskName, projectName) {
    const currentDate = new Date();
    ...
    if (checkDate <= currentDate) {
        return true; // Allow all entries for past and present dates
    }

    // Allow only "Leave" task for "Elastik Teams" project in the future
    return taskName === 'Leave' && projectName === 'Elastik Teams';
}
